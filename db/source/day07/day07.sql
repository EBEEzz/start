        ...
        형식2)
            ...
            주의) 컬럼이 만들어진 이후에는 NOT NULL 제약조건은 추가하지 못한다.
                => 제약조건을 추가하지 않고 테이블을 만들게 되면
                    컬럼들은 NULL데이터를 허용하는 컬럼으로 만들어진다.
                    따라서 NOT NULL 제약조건은 이러한 컬럼의 속성을 수정해야한다.
                    
        형식3) 제약조건 없이 테이블을 만드는 경우 테이블 수정명령으로 제약조건을 추가하는 방법
...
--------------------------------------------------------------------------------
        ...
        3) 필드 길이 변경하기
            ...
            *****
            참고) 길이변경은 현재길이보다 늘이는 것은 가능하지만,
                줄이는 것은 불가능하다.
                => 이미 입력되어있는 데이터가 수정될 길이를 넘었을 가능성이 있어서
...
--------------------------------------------------------------------------------
    ...
    테이블 삭제하기
        ...
        참고) DML 명령은 복구가 가능하지만
            DDL 명령은 복구가 원칙적으로 불가능하다.
            
        참고) 10g부터 휴지통 개념을 추가해서
            삭제된 데이터를 휴지통에 보관하도록 하고있다.
...
/*
    TRUNCATE
    => DML 명령 중 DELETE 명령과 같이 테이블 안에 있는 모든 데이터를 삭제하는 명령
    
    형식)
        TRUNCATE TABLE 테이블이름;
        
    참고) DELETE 명령은 DML 소속 => 복구가 가능하다.
        TRUNCATE 명령은 DDL 소속 => 복구가 불가능하다.
*/

/*
    무결성 체크
    => 데이터베이스는 프로그램 등 전산에서 작업할 때 필요한 데이터를 제공해주는
        보조프로그램이다.
        따라서 데이터베이스가 가진 데이터는 항상 완벽한 데이터여야한다.
        그런데 데이터를 입력하는 것은 사람의 몫이고 완벽한 데이터를 보장할 수 없다.
        
        각각의 테이블에 들어가서는 안 될 데이터나 빠지면 안 되는 데이터 등을
        미리 결정해놓고 데이터를 입력하는 사람이 잘못입력하면
        그 데이터는 아예 입력되지 못하도록 방지하는 역할의 기능이다.
        
        한 마디로 정리하자면 "이상데이터가 입력되는 것을 방지하는 기능" 이다.
        
        따라서 이 기능은 반드시 필요한 기능은 아니다.
        (=> 입력하는 사람이 정신차리고 입력하면 되기 때문에)
        실수를 미연에 방지할 수 있도록 도와주는 기능이다.
*/

/*
    참고) 테이블을 생성하는 명령으로 서브질의의 결과를 이용해서
        테이블을 만드는 방법도 있다.
        ...
        참고) 이렇게 복사하게되면 모든 데이터와 테이블의 구조를 복사할 수 있지만
            NOT NULL 제약조건을 제외한 모든 제약조건을 복사해오지 않는다.
            
        참고) 이때 복사할 테이블의 구조만 복사하고 싶은 경우
            CREATE TABLE 테이블이름
            AS
...
/*
    DCL(Data Control Language)
        ...
        트랜젝션 처리
        => 교과서적인 의미로 오라클이 처리(실행)하는 명령 단위
        
            우리가 테이블을 만들 때 create 명령을 치고 엔터를 누르면
            그 명령을 실행하게 되는데 이것을 "트랜젝션이 실행되었다."라고 표현한다.
            
            위와같이 대부분의 명령은 엔터키를 누르는 순간 실행되고
            그것은 곧 트랜젝션이 실행된 것이므로
            결국 오라클은 명령 한 줄이 곧 하나의 트랜젝션이 된다.
            
            그런데 DML 명령만큼은 트랜젝션 단위가 달라진다.
                참고) DML 명령 : INSERT, UPDATE, DELETE
            => DML 명령은 곧장 실행되는 것이 아니고 버퍼장소(임시 기억장소)에
                그 명령을 모아만 놓는다.
                => 결국 트랜젝션이 실행되지 않는다.
                
            따라서 DML 명령은 강제로 트랜젝션 실행 명령을 내려야한다.
            이때 트랜젝션은 한 번만 일어나게 된다.
            
            왜?
                DML 명령은 데이터를 변경하는 명령이다.
                데이터베이스에서 가장 중요한 개념은 무결성인데 이런 곳에 DML 명령이
                한순간 트랜젝션 처리가 된다면 데이터의 무결성이 깨질 수 있다.
                이런 문제점을 해결하기위한 목적으로 트랜젝션 방식을 변경해놓았다.
                
            그런데 버퍼에 모아놓은 명령을 트랜젝션 처리하는 방법에는
    ...
    *****
    ...
    COMMIT 이라고 명령해서 트랜젝션을 발생시키고 만약 검토과정 중 오류가 발견되면
...
/*
    뷰(VIEW)
    => 교과서적인 의미로 질의명령의 결과를 하나의 창문으로 바라볼 수 있는 창문을 의미
...
--------------------------------------------------------------------------------
    ...
    참고) 원칙적으로 사용자 계정은 관리자가 허락한 일만 할 수 있다.
        그런데 뷰를 만드는 권한은 아직 부여되어있지않다.
        따라서 권한을 부여하고 뷰를 만들어야한다.
        
        권한 부여하는 방법 - 관리자 계정에서(SYSTEM)
...
--------------------------------------------------------------------------------
    뷰를 만드는 방법
        ...
        형식2)
            => DML 명령으로 데이터를 변경할 때 변경된 데이터는 기본 테이블만 변경.
        ...
        형식3)
            => 뷰를 이용해서 데이터를 변경하면 뷰를 사용하는 데이터만 변경 가능.
                이것은 원본 테이블의 입장에서 문제가 발생할 수 있다.
...
--------------------------------------------------------------------------------
    ...
    참고) 원래 뷰는 기본 테이블이 있고 그것을 바라보는 창문을 만드는 개념이다.
        ...
        참고) 진짜로 테이블이 없이 부가 작동되는 것은 아니다.
            테이블은 필요한데 이 명령을 내리는 순간만 없는 경우에 
            급할 때 사용하는 방법
...
-- EMP 테이블의 사원번호, 이름, 부서번호, 부서이름, 부서위치를 조회하는 뷰(EMP_DNO)를 만드세요.
...
-- 데이터 추가(X : 두 테이블을 조인해서 만든 복합뷰이므로 DML 명령을 사용할 수 없다.)
INSERT INTO
    EMP_DNO
VALUES(
...
/*
    문제2)
        10번 부서 사원들의 사원번호, 이름, 급여, 커미션을 조회하는 뷰(DNO10)
*/
...
ALTER TABLE EMPLOYEE
ADD CONSTRAINT EMPLO_DNO_FK FOREIGN KEY(deptno) REFERENCES dept(deptno);

/*
    문제3)
        EMPLOYEE 테이블의 10번 부서 사원들의 사원번호, 이름, 급여, 커미션을 조회하는 뷰(DNO10)
