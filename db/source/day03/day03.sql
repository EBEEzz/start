/*
    ...
    10. TRIM
        ...
        형식) 
            TRIM([삭제할문자 FROM] 데이터)      []부분 생략가능
        
        참고) 같은 문자가 앞, 뒤에 연속되어 있으면 모두 지운다.
        
        참고) 가끔 데이터 앞 또는 뒤에 공백문자가 들어간 경우가 있다.
            이런 경우를 대비해서 앞 뒤에 들어간 공백문자를 제거할 목적으로 많이 사용한다.
    ...
    11. CHR()
        => ACSII 코드를 알려주면 그 코드에 해당하는 문자를 알려주는 함수
    ...
    12. ASCII()
        ...
        참고) 두글자 이상으로 된 문자열의 경우는 첫문자의 코드값을 반환해준다.
        
    13. TRANSLATE
        => REPLACE와 마찬가지로 문자열 중 지정한 부분을
            다른 문자열로 바꿔서 반환해주는 함수
            
            차이점) REPLACE 함수는 바꿀 문자열 전체를 바꾸는데
                    이 함수는 문자단위로 처리한다.
...
SELECT
    LTRIM('ooooooooooooooooooooooracleooooooooooooooooooooo', 'o')
FROM
    dual
;
...
SELECT
    ...
    TRIM('o' FROM 'ooooooooooooooooooooooracleooooooooooooooooooooo') TRIM
FROM
    dual
;
...
/*
    날짜 처리 함수
        ...
        **
        ...
        참고) 날짜 - 날짜 의 연산식이 가능하다. (날짜연번끼리 - 연산을 하게된다.)
            
            참고) 날짜연번 : 날수.시간
            
            날짜 데이터의 기준점은 1970년 1월 1일 0시 0분 0초
        ...
        참고) 날짜 +(또는 -) 숫자 의 연산은 가능하다.
            => 날짜연번 +(-) 숫자 이므로 원하는 숫자만큼 이동된 날짜를 표시한다.
*/
...
/*
    날짜데이터 처리함수
        ...
        1. ADD_MONTHS()
            => 날짜에 지정한 달 수를 더하거나 뺀 날짜를 알려준다.
            
            형식)
                ADD_MONTHS(날짜, 더할개월수)
                
            참고) 더할 개월 수에 음수를 입력하면 뺀 날짜를 알려준다.
            
        2. MONTHS_BETWEEN
            => 두 날짜 데이터의 개월 수를 알려주는 함수
        ...
        4. NEXT_DAY
            ...
            형식)
                NEXT_DAY(날짜[혹은 '날짜'], '요일')
                
                참고) 요일 정하는 방법
                    1. 우리는 한글세팅이 된 오라클이므로
                        '월', '화', ...
                        '월요일', '화요일', ...
                    2. 영문권에서는
                        'SUN', 'MON', ...
...
SELECT
    ename 이름, sal 급여, hiredate 입사일, LAST_DAY(hiredate) 첫급여일
FROM
    emp
;
...
SELECT
    ename 이름, sal 급여, hiredate 입사일, (LAST_DAY(hiredate) + 1) 첫급여일
FROM
    emp
;
...
-- 올 성탄절 이후 첫 토요일은?
SELECT
    NEXT_DAY('2022/12/25', '토') AS "첫 토요일"
    -- 이 경우는 날짜 데이터를 입력해야 하지만 문자로 입력해도 실행된다.
    -- 문자 데이터를 날짜 데이터로 변환해주는 함수가 자동호출되었기 때문에 가능하다.
FROM
    dual
;
    ...
    2. TO_DATE()
        ...
        형식2)
            ...
            => 문자열이 오라클이 지정하는 형식의 날짜처럼
                ...
                참고) 여기에서의 변환형식이란?
                    입력한 문자데이터가 어떤 의미를 가지고 만들었는지
                    알려주는 기능을 한다.
    
    3. TO_NUMBER()
        => 숫자형식의 문자데이터를 숫자로 변환시켜주는 함수
            문자데이터는 +, - 연산이 안되기때문에 필요함.
...
/*
    사원들의 이름, 입사일, 부서번호를 조회하세요.
    단, 입사일은 '0000년 00월 00일'의 형식
*/
SELECT
    ename 이름,
...
-- 사원 급여를 조회하는데 앞에는 $를 붙이고 세자리마다 , 를 붙여서 조회하세요.
SELECT
    ename 이름, sal, TO_CHAR(sal, '$9,999,999') 문자급여1,
...
-- 자신이 지금까지 며칠동안 살고있는지를 알아보자.
-- 힌트) 현재시간 - 생년월일
SELECT
    FLOOR(sysdate - TO_DATE('98/03/20')) "살아온 날 수", 
    FLOOR(sysdate - TO_DATE('98 03 20'))
FROM
    dual
;

SELECT FLOOR(SYSDATE - TO_DATE('03/20/1998', 'MM/DD/YYYY')) FROM dual;

SELECT FLOOR(SYSDATE - TO_DATE('03201998', 'MMDDYYYY')) FROM dual;

--------------------------------------------------------------------------------
-- TO_NUMBER

-- '123'과 '456'을 더한 결과를 조회하세요.
SELECT '123' + '456' result FROM dual;  -- 이 경우 형변환 함수가 자동호출되었다.

SELECT TO_NUMBER('123') + TO_NUMBER('456') RESULT FROM dual;    -- 정석

SELECT TO_NUMBER('1,234', '9,999') + TO_NUMBER('5,678', '9,999,999') FROM dual;
...
/*
    ...
    1. NVL()
        => NULL 데이터는 모든 연산(함수)에 적용되지 못한다.
            ...
            의미) NULL 데이터이면 강제로 지정한 데이터로 바꾸어서
                연산, 함수에 적용하도록 하는 함수
            ...
            ***
            주의) 지정한 데이터와 바뀔 내용은 반드시 형태가 일치해야 한다.
            
    2. NVL2()
        형식)
            NVL2(필드이름, 처리내용1, 처리내용2)
            
        의미) 필드의 내용이 NULL이면 처리내용2를
    ...
    4. COALESCE()
        ...
        의미) 여러개의 데이터 중 가장 첫번째 나오는 NULL이 아닌 데이터를 출력하세요.
*/
SELECT
    ename 이름,
...
/*
    문제1) comm이 존재하면 현재 급여의 10%를 인상한 금액 + 커미션을
        존재하지 않으면 현재급여의 5%를 인상한 금액 + 100 으로 조회하세요.
        
        조회내용은 사원이름, 급여, 커미션, 지급급여
*/
SELECT
...
/*
    문제2) 커미션에 50%를 추가해서 지급하고자 한다.
        만약 커미션이 존재하지 않으면 급여를 이용해서 10%를 지급하고자 한다.
        사원들의 이름, 급여, 커미션, 지급급여를 조회하세요.
*/
SELECT
    ename 이름, sal 급여, comm 커미션,
--    COALESCE((sal + comm * 1.5), (sal * 1.1)) 지급급여
    NVL2(comm, (sal + comm * 1.5), (sal * 1.1)) 지급급여
FROM
    emp
;
...
/*
    ...
    1. DECODE : switch ~ case 명령에 해당하는 함수
        ...
        의미) 필드의 내용이 값1과 같으면 처리내용1을 값2와 같으면 처리내용2를
                ...
            그 이외의 값은 처리내용n 으로 처리하세요.
...
/*
    급여가 1000 미만이면 20%를 인상하고
            ...
            3000 이상이면 10% 인상한 사원들의 이름, 직급, 급여, 인상급여 조회.
*/
SELECT
    ...
    => 여러행의 데이터를 하나로 만들어서 뭔가를 계산하는 함수
        ...
        ***
        ...
        => 데이터들의 갯수를 구해주는 함수
            ...
            참고) 필드이름 대신 * 를 사용하면 각각의 필드의 카운터를 따로 구한 후
                그 결과 중에서 가장 큰 값을 알려주게 된다.
            
        4. MAX / MIN
        => 지정한 필드의 데이터 중에서 가증 큰 값(또는 작은 값)을 알려주는 함수
...
/*
    ...
    => 그룹 함수에 적용되는 그룹을 지정하는 것
        조회를 할 때 대상을 그룹핑해서 조회하는 방법
        
        예) 부서별로 급여의 합계를 구하고 싶다.
    ...
    참고) GROUP BY를 사용하는 경우 GROUP BY에 적용된 필드는 같이 조회할 수 있다.
    
    HAVING
    => 그룹화 한 경우 계산된 그룹 중에서 출력에 적용될 그룹을
        지정하는 조건식을 기술하는 절
        
        ***
        참고) WHERE 조건절은 계산에 포함된 데이터를 선택하는 조건절이다.
        HAVING 조건은 그룹화해서 계산한 후 출력할지 말지 결정하는 조건절이다.
        
        *****
        참고) where 절 안에서는 그룹함수를 사용할 수 없다.
            하지만 having 절 안에서는 그룹함수를 사용할 수 있다.
*/

-- 직급별 급여평균을 직급명과 함께 조회하세요.
SELECT
    job 직급, FLOOR(AVG(sal)) 부서평균급여
FROM
    emp
GROUP BY
    job
...
10	2916
*/
-- 부서별 평균 급여를 조회하세요. 단, 부서평균 급여가 2000 이상인 부서만 조회
SELECT
