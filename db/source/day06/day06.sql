/*
    ...
    목적) 데이터베이스의 이상현상을 제거할 목적으로 사용
    
    종류)
        기본키(PRIMARY KEY) 제약조건
            => 속성값으로 데이터를 구분할 수 있어야한다.
                기본키 제약조건은 필수는 아니지만 되도록이면 추가하는 것이 좋다.
                
                기본키는 유일키 + NOT NULL 제약조건
                
        유일키(UNIQUE) 제약조건
            => 속성값이 다른 데이터와 구분이 되어야한다.
                NULL 데이터는 입력할 수 있다.
            
        참조키(외래키, FOREIGN KEY) 제약조건
            => 참조하는 테이블의 키값을 반드시 사용해야하는 제약조건
                입력되지 않은 키값은 입력하지 못하도록 막는 제약조건
                테이블을 삭제하려면 먼저 참조하고있는 테이블을 삭제하고
                해당 테이블을 삭제해야한다.
...
/*
    과제) (제출여부 상관X)
        방명록을 만드려고 한다.
        회원가입을 하면 방명록 페이지로 자동 전환되도록
        방명록에는 회원아이디와 인삿말이 포함되어야한다.
        (방명록은 한 사람이 글 하나만 작성가능하고 댓글도 글제목도 없다.)
        
        방명록의 ER Model Diagram, ER-D, 테이블 명세서, DDL 질의명령을 작성하세요.
*/
...
/*
    오라클에서 사용할 수 있는 데이터 형태
        ...
        1. 문자형
            CHAR
                ...
                형식)
                    CHAR(최대길이)
                    => 이때 사용가능한 최대 길이는 2000바이트까지만 사용할 수 있다.
            ...
            CLOB
                ...
                형식)
                    CLOB
                    => 최대 길이는 4Gb까지 사용할 수 있다.
                    
                => 이 타입은 VARCHAR2가 4Kb까지만 허락하기 때문에
                    그 이상의 글자수를 가진 데이터는 입력받지 못한다.
                    이 문제를 해결하기 위한 방식으로 만들어놓은 타입이다.
                    
    참고)
        고정길이 문자형 (처리속도가 가변길이 문자형보다 빠르다.)
            => 문자의 길이를 미리 지정한 후 입력한 문자가 길이보다 부족하면
                공백을 이용해서 반드시 같은 길이의 문자를 만들어서 입력하는 방식
                => 우편번호와 같이 길이가 완전히 고정된 데이터에 한해 주로 사용.
            
        가변길이 문자형
            => 입력한 문자의 길이에 따라서 스스로 사용하는 공간을 변경해서
                문자열의 길이를 입력한 문자의 길이에 맞추는 형태
                => 입력될 데이터의 사이즈를 특정할 수 없을 때 주로 사용한다.
    ...
    참고) 데이터베이스에 따라 데이터 형태도 약간씩 달라진다.
        요사이 ANSI 데이터 형태라고 해서 모든 데이터베이스에 적용 가능한
        공통 형태를 표준화협회에서 제시하고 있다.
...
/*
    제약조건 부여하는 방법
        ...
        1. 테이블 생성할 때 추가하는 방법
            ...
            1) 필드를 정의할 때 추가하는 방법 (*** 권장 ***)
                형식)
                    필드이름 데이터타입(길이)
                        CONSTRAINT 제약조건이름 제약조건1
                        CONSTRAINT 제약조건이름 제약조건2     << 여러개인 경우
            ...
            2) 필드를 미리 정의하고 나중에 제약조건을 추가하는 방법
            
            3) 무명 제약조건으로 등록하는 방법 (권장하지 않음)
*/
...
CREATE TABLE memb(
    ...
    addr VARCHAR2(100 CHAR)
    CONSTRAINT MEMB_ADD_NN NOT NULL,
    gen CHAR(1)
