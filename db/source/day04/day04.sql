...
/*
    ...
    => 질의명령안에 다시 질의명령을 포함하는 경우
        ...
        이때 위의 질의명령을 아래 메인 질의명령에서 조회해서 사용해야 한다.
        
    참고) 이때 서브질의를 감싸는 질의명령을 메인질의명령이라고 부른다.
    
    서브질의의 위치에 따른 결과
        1. SELECT 절
        => 이 부분에 사용되는 질의명령은 반드시 결과가 한 행, 한 필드가 나와야한다.
        
        2. FROM 절
        => FROM 절에는 테이블이 나열되어야 한다.
            조회 질의명령의 결과는 마치 테이블과 같다.
            이러한 조회 질의명령의 결과를 테이블처럼 사용할 수 있는 방법
            
            이때 FROM 절 안에 들어가는 서브질의를 마치 테이블과 같다해서
            INLINE TABLE 이라고 부른다.
            
            질의명령을 보낼 때 사용한 별칭을 사용해서 데이터를 꺼내야한다.
        ...
        3. WHERE 절
            ...
            2) 다중행 단일필드로 결과가 발생하는 경우
                참고) 서브질의의 결과가 다중행 단일필드로 발생하는 경우
                    이때 사용하는 연산자
...
FROM
    emp
    /*
        테이블을 가리킬 때 원칙은 계정.테이블이름(scott.emp) 의 형식.
        하지만 접속계정이 가지고 있는 테이블에 한해서는 테이블이름만 기술해도된다.
    */
WHERE
...
SELECT
    ename 이름, deptno 부서번호, cnt 부서원수, avg 부서평균급여, sum 부서급여합계
FROM
    emp,
    (
        SELECT
