...
/*
    참고)
        오라클이 데이터를 보관하는 방법
            ...
            참고)
                ...
                이런 종류의 데이터베이스 관리 시스템을
                    RDBMS - 개체들 간에 관계를 형성해서 데이터를 관리하는 시스템
                            관계형 데이터베이스 관리 시스템
        ...
        참고로 필드에는 그 항목을 구분하기 위한 이름이 부여되어 있따.
        이것을 우리는 필드이름이라고 부른다.
        하지만 레코드는 각 행을 구분하는 방법이 존재하지 않는다.
        
    참고)
        우리가 오라클에 접속하게 되면 오라클이 접속자에게 메모리를 할당해준다.
        이때 이 메모리가 할당된 상태를
            "세션이 하나 열렸다."   라고 표현한다.
        ...
        따라서 접속한 사람은 확보한 공간에서만 작업(DML명령)을 하게되고
        최종적으로 데이터베이스에 적용시키는 작업은 별도로 명령(TCL명령)해야한다.
*/
...
/*
    ...
    주의사항)
        이 명령은 질의명령에서 한번만 사용해야하고 조회된 데이터의
        각 행들이 같은 경우에만 적용된다.
...
SELECT
    ename as 사원이름, sal AS 원급여, sal * 1.1 "인상 급여" -- 공백이 포함된 별칭은 큰따옴표로 감싸줘야한다.
FROM
    emp
;
...
/*
    참고) DUAL 테이블
        ...
        필터링을 거쳐 그 결과를 가지고 보여주게 된다.
        그런데 데이터 자체를 조회하게되면(SELECT절에 데이터를 나열하면)
        조회되는 데이터는 필터링된 데이터의 갯수만큼 출력될 것이다.
        ...
        하지만 이 테이블은 물리적으로 저장되어있는 테이블은 아니고
...
/*
    NULL 데이터
    => 필드 안에는 데이터가 보관되어야하는데 없는 정보는 데이터가 없을 수 있다.
    ...
    ***
    참고) NVL 함수
        ...
        의미)
            ...
            따라서 특정 데이터로 변경하게되면 연산이 가능해진다.
        
*/

-- 사원들의 상사번호에 10000을 추가해서 사원이름, 상사번호를 조회하세요.
SELECT
    ename 사원이름, mgr + 10000 상사번호
FROM
    emp
;
...
-- 사원번호, 사원이름을 조회하는데 0000번, 홍길동 님 의 형식으로 조회하세요.
SELECT
...
/*
    ...
    참고)
        ...
        => NULL 데이터는 모든 연산에서 제외된다.
            ...
            따라서 NULL 데이터의 비교는 
                IS NULL, IS NOT NULL    을 사용해서 비교해야한다.
*/
...
FROM
    emp
WHERE
    comm IS NOT NULL    -- NOT comm IS NULL 도 같은 의미
;
...
ORDER BY
    hiredate --ASC
;
--  참고) 정렬할 때 오름차순 정렬의 경우 ASC를 생략해도 된다.

-- 사원들의 이름, 급여, 부서번호를 조회하는데
-- 부서번호 기준 오름차순 정렬하고 같은 부서의 경우 급여가 많은 사람이 먼저 출력되게하세요.
SELECT
...
/*
    ...
    => 두 개 이상의 SELECT 질의명령을 이용해서 그 결과의 집합을 얻어내는 방법
        ...
        종류)
            ...
            UNION ALL
            => 합집합의 개념
                단, UNION의 경우 중복데이터를 한번만 출력해주는데
                UNION ALL의 경우 중복데이터를 모두 출력해준다.
            
            INTERSECT
            => 교집합의 개념
                조회 질의명령 결과 중 양쪽 모두 존재하는 결과만 출력하는 것
                
            MINUS
            => 차집합의 개념
                앞의 질의명령의 결과에서 뒤의 질의명령의 결과에 포함된 데이터를
                뺀 결과를 출력해준다.
                
    참고) 공통적인 특징
        1. 두 질의명령에서 나온 결과는 같은 필드의 갯수를 가져야한다.
...
/*
    ...
    *****
    주의사항)
        단일행 함수, 일반필드와 그룹함수는 절대로 같이 사용할 수 없다.
    ...
    단일행 함수
        ...
        ***
        ...
        => 데이터가 숫자인 경우에만 사용할 수 있는 함수
            ...
            2) ROUND() : 반올림해주는 함수 => 지정한 자릿수에서 반올림
                형식)
                    ROUND(데이터[, 자릿수])
            ...
            4) TRUNC() : 자릿수 이하 버림 함수, 자릿수를 지정해서 버릴 수 있다.
                형식)
                    ...
                    참고) 자릿수는 소수이하 자릿수를 의미하고,
                        만약 음수형태로 입력하면 소수이상 자릿수를 의미한다.
...
SELECT
    ename 사원이름, sal 원급여, 
    sal * 1.15 계산값, ROUND(sal * 1.15, 0) 인상급여, 
...
/*
    2. 문자 처리함수
        ...
        8. LPAD() / RPAD()
        => 문자열의 길이를 지정한 후 문자열을 만드는데
            남는 공간은 지정한 문자로 채운 문자열을 만들어주는 함수
            차이점은 남는 공간을 채울 때
                왼쪽을 채울건지        LPAD
                오른쪽을 채울건지      RPAD
...
-- 사원들의 이름, 직급, 급여를 조회하는데   Mr.SMITH    CLERK 직급    800 달러
-- 형식으로 출력되게 하세요.
SELECT
...
WHERE
    INSTR(ename, 'a') = 0
    -- 혹은 ename NOT LIKE '%a%' 로도 풀 수 있지만 문자열 처리함수가 아니다.
;
...
*/
SELECT
    LPAD(SUBSTR(ename, -2), LENGTH(ename), '#') 사원이름, hiredate, sal
FROM
    emp
;
