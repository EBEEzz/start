-- 계층형 질의
/*
    댓글형 게시판의 경우 목록을 꺼내올 때 상위글 바로 다음에
    그 글의 댓글이 조회되어야 한다.
...
/*
    ...
    이제까지 우리가 배운 명령들은 모두 CRUD 중 R에 해당하는
    SELECT 질의명령만 공부했는데
    
    C - CREATE - INSERT
    U - UPDATE - UPDATE
    D - DELETE - DELETE     을 공부할 예정이다.
    ...
    1. INSERT
        => 이미 만들어진 테이블에 데이터를 추가하는 명령
            ...
            형식)
                ...
                VALUES(
                    테이블이 가지는 모든 필드의 데이터
                        만약 필드이름을 지정해 놓으면 위에서 지정한 순서대로
                        필드에 해당하는 데이터를 입력해야한다.
                );
                
            주의) 입력을 할 때 데이터의 갯수와 순서를
                  반드시 필드의 순서에 맞게 입력해야한다.
                  
        참고) 만약 데이터가 준비되지 않아서 데이터가 부족할 경우에는
...
/*
    **********
    참고) DML 명령은 메모리 상에 확보한 세션공간에서만 작업이 이루어진다.
        ...
        작업내용이 적용되기를 원하면 반드시 commit 명령을 실행해야한다.
*/
...
/*
 ...
 이때 여러개릐 필드를 동시에 질의명령의 결과로 대체하고자 한다면
    ...
    참고) 만약 수정을 원하지 않는 데이터를 반드시 써야한다면
        현재 데이터를 그대로 덮어씌우는 것도 가능하다.
*/
...
/*
    DELETE
    => 현재 데이터 중에서 필요하지 않은 데이터를 삭제하는 명령
        한 행을 통째로 지운다는 의미
...
/*
    참고) 테이블 설계
        ...
        그 데이터에 한해서만 기억하도록 하는 데이테베이스이다.
        
        따라서 테이블을 만들 때는 여러가지 규칙을 정해놓고(정규화 규칙)
        테이블에 들어갈 데이터를 결정해야한다.
        ...
        데이터베이스 설계과정
            ...
            2. 논리적 설계
                => 개념적 설계의 결과를 가지고 물리적 저장장치에 저장할 수 있도록
