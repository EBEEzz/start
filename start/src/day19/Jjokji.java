...
public class Jjokji {
	...
	/*
	 	...
	 	다만 하나를 공유하는 경우는 보내는 도중에는 받지 못할 수 있다.
	 	그러므로 안전한 통신을 위해서는 소켓을 구분하여 처리하는 것이 좋다.
	 */
	public Jjokji() {
		setMap();
		...
		/*
		 	...
		 	여기서는 UDP 통신을 해야하므로 DatagramSocket을 이용해서 네트워크 구성을 해야한다.
		 */
		try {
			sSocket = new DatagramSocket();
			rSocket = new DatagramSocket(3208);
			/*
			 	이렇게 하면 누군가에게 접속한 것이 아니고 네트워크 회선에만 접속한 상태가 된다.
			 	=> 통신이 가능하도록만 해주는 것이다.
			 */
			
			//이제 네트워크 구성이 완료되었으므로 쪽지를 보내고 받을 수 있다.
			//받는 프로그램을 시작하자.
			//받는 프로그램은 다른 프로그램들과 병행해서 처리가 되어야 하므로 스레드로 구성한다.
			
			ReceiveThread t = new ReceiveThread(this);	//New Born
			
			//Runnable
			t.start();
			
		} catch(Exception e) {
			//위 작업이 예외가 발생한 것은 이 프로그램이 더이상 제 기능을 할 수 없는 상태이므로
			//열려있는 소켓을 모두 닫아준다.
			close();
			e.printStackTrace();
		}
	...
	public void setMap() {
		...
		try {
			fin1 = new FileInputStream("src/day19/nameIp_b.properties");
			fin2 = new FileInputStream("src/day19/ipName_b.properties");
