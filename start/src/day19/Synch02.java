...
import java.util.*;
/*
 	ArrayList 하나를 준비해서 이곳에 데이터를 넣고 빼고를 할 예정이다.
...
public class Synch02 {
	...
	synchronized void makeData(int num) {
		if(list.size() == 5) {
			...
			try {
				...
				wait();
				//이 순간 이 함수를 사용하는 스레드는 스스로 Block 상태로 전위된다.
			} catch(Exception e) {}
		}
	...
	synchronized void useData() {
		...
		int no = list.get(0);
		//꺼낸 데이터는 리스트에서 제거한다.
		list.remove(0);		//꺼낸 데이터를 제거했으므로 입력할 수 있는 공간이 하나 더 생겨났다.
		System.out.println("\t\t" + no + " 를 꺼냈습니다.");
		System.out.println("\t\t남아있는 데이터의 갯수는 " + list.size() + " 개 남았습니다.");
		//이제 데이터를 추가할 수 있는 공간이 추가되었으니 다시 데이터를 채우는 작업을 깨우러 간다.
		notify();	
	}
