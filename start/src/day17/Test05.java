...
public class Test05 {
	...
	public Test05() {
		...
		for(File f : infoList) {
			...
			String pold = f.getParent();	//상위폴더 이름 꺼내주는 함수
			
			//출력
			System.out.println("상위폴더 : " + pold + " , 파일 이름 : " + fName + " , 사이즈 : " + len + " byte");
		}
		*/
		
		//확장자가 jpg인 파일만 골라서 출력하고싶다.
		String[] flist = dir.list(new FilenameFilter() {
			...
			public boolean accept(File dir, String name) {
				...
				if(name.endsWith("jpg")) {	//파일 이름이 jpg로 끝나는지
					//이 경우는 결과 목록에 이 파일을 포함시킨다.
					bool = true;
				}/* else {
					bool = false;
				}
				//bool 변수는 초기값으로 이미 false로 세팅되어있으므로 생략해도 무방하겠다.
				*/
