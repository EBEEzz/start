...
public class Test04 {
	public Test04() {
		...
		Figure f2 = nemo;
		/*
		 	이 경우 삼각형, 사각형에 있는 getWidth(), getHeight() 는 Figure의 멤버로는
		 	존재하지 않으므로 이렇게 Figure 타입의 변수에 삼각, 사각의 객체를 담는 경우는
		 	두 함수를 호출할 수 없다.
		 */
		System.out.printf("넓이가 %6.2f 인 삼각형\n", f1.getArea());
		System.out.printf("넓이가 %6.2f 인 사각형\n", f2.getArea());
		//결론)	상위클래스 타입의 변수에 하위클래스의 인스턴스를 담는 경우,
		//		상위클래스에 없는 멤버는 사용할 수 없다.
	}
