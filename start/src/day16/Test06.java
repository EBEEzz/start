...
/*
 	...
 	추가될 친구들은 그룹, 이름, 전화번호 로 입력할 예정이다.
...
public class Test06 {
	...
	public void makeGroup(String name) {
		//그룹이름을 알려주면 그 그룹을 만들어주는 함수
		if(!groupMap.containsKey(name)) {
			//지금 만들고자 하는 그룹이 존재하지 않으면 => 새롭게 그룹을 만들어줘야 한다.
	...
	public void setFriend(String group, String name, String tel) {
		...
		//만약 등록하는 그룹이 맵에 존재하지 않으면 새롭게 그룹을 만들어서 추가
		makeGroup(group);
	...
	public void getGroup(String group) {
		//1. 꺼낼 그룹의 맵을 알아낸다.
		HashMap tmp = (HashMap) groupMap.get(group);
		//2. 그룹내에 몇명이 누가 들어있는지 알 수 없으므로 키값만 꺼내와서
		Set set = tmp.keySet();
		//3. 꺼내온 키값으로 데이터를 꺼낸다.
